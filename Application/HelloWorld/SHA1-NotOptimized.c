...
UINT32 self_test(void)
{
  ...
  for (i = 0; i < sizeof(testCases) / sizeof(testCases[i]); ++i) {
    ...
    sha1_start(&s);
    {
        ...
        DEBUG((EFI_D_ERROR, "len is %d\t ", len));
        Print (L"Test case %d len is %d", i, len);
    ...
    slow_sha1_update( &ctx, (uint8*)tc->msg, (UINT32)AsciiStrLen(tc->msg) );
    slow_sha1_finish( &ctx, hash );
...
//  Print (L"Elapse time(stall 0.1s): %d \n", timeStamp/Frequency);
  ...
  timeStamp = AsmReadTsc();
  for (i = 0; i < N; i++) {
    slow_sha1_process( &ctx,  (uint8_t *)block );
  }
  timeStamp = AsmReadTsc() - timeStamp;
  ...
  Print (L"SHA-1 implementation @ Christophe Devine version\n Speed: %d.%d MB/s \n", SpeedInMB/1000, SpeedInMB%1000 );
#endif  
  return;
}
